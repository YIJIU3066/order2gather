              ...
              <div className='w-full '>
                <div className='w-full my-2 p-1 flex border border-gray-200 bg-slate-50 rounded '>
                  <div className='w-full flex flex-auto flex-wrap'>
                    {!isGroup && !selectedItems.length && (
                      <p className='my-auto mx-1 text-grey opacity-50 text-lg font-medium'>
                        Select Friend
                      </p>
                    )}
                    {isGroup && !selectedItems.length && (
                      ...
                      selectedItems.map((tag, index) => {
                        if (index < 2) {
                          return (
                            <div
                              key={index}
                              className='flex justify-center items-center m-1 font-medium py-1 px-2 rounded-lg text-white bg-green'
                            >
                        ...
                        if (index < 3) {
                          return (
                            <div
                              key={index}
                              className={
                                'flex justify-center items-center my-auto font-bold h-6 w-6 rounded-full border-green border border-[3px] text-grey bg-slate-50 ' +
                                (index === 1
                                  ? '-translate-x-2'
                                  : index === 2
                                    ? '-translate-x-4'
                    ...
                    {isGroup && selectedItems.length > 2 ? (
                    ...
                    {!isGroup && selectedItems.length > 3 ? (
